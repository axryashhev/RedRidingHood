(function() { // module pattern
  ...
  var MAP      = { tw: 64, th: 48 },
      ...
      SPRITE_GAME = { PLAYER: 'player.png', MONSTER: 'monster.png', HEALTH: 'heart.png'},
      COLOR    = { BLACK: '#000000', YELLOW: '#f7ff08', GREEN: '#0dcd2b', PINK: '#C02942', PURPLE: '#35ff08', ORANGE: '#ff9008', SLATE: '#53777A', RED: 'red' },
      COLORS   = [ COLOR.YELLOW, COLOR.GREEN, COLOR.YELLOW, COLOR.PURPLE, COLOR.ORANGE ],
  ...
  var fps      = 60,
      ...
      treasure = [],
      cells    = [],
      background = new Image(canvas.width, canvas.height),
      sprite_player = new Image(TILE, TILE),
      sprite_moster = new Image(TILE, TILE),
      sprite_health = new Image(TILE, TILE);
      // here sprites are assigned an image
      background.src = 'Background/background-1.png';
  ...
  function renderPlayer(ctx, dt) {
     ...
     // ctx.fillStyle = COLOR.YELLOW;    console.log(sprite_player);

     ctx.drawImage(sprite_player, player.x + (player.dx * dt), player.y + (player.dy * dt));
  ...
  function renderMonsters(ctx, dt) {
    ...
    for(n = 0, max = monsters.length ; n < max ; n++) {
      monster = monsters[n];
      if (!monster.dead)
        ctx.drawImage(sprite_moster, monster.x + (monster.dx * dt), monster.y + (monster.dy * dt));
  ...
  function renderTreasure(ctx, frame) {
    ...
    for(n = 0, max = treasure.length ; n < max ; n++) {
      t = treasure[n];
      if (!t.collected)
        ctx.drawImage(sprite_health, t.x, t.y + TILE/3);
